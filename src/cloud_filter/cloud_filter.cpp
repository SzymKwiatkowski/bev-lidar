...
void CloudFilter::initMaxPointsMap(int grid_dim, float cell_size, float z_min, float z_max, int num_slices, int planes,
    ...
    python_cmd_ << " --maps " << map_path_;
    python_cmd_ << " --map_size " << grid_dim;
    python_cmd_ << " --cell_size " << cell_size;
    python_cmd_ << " --min_height " << z_min;
    python_cmd_ << " --max_height " << z_max;
    python_cmd_ << " --num_slices " << num_slices;
    python_cmd_ << " --num_planes " << planes;
    python_cmd_ << " --velo_minangle " <<  low_angle;
    python_cmd_ << " --velo_hres " << h_res;
    python_cmd_ << " --velo_vres " << v_res;
    python_cmd_ << " --velo_height " << velo_h.str();
    std::cout << "Required max_points map not found, creating map..." << std::endl;
    std::cout << python_cmd_.str() << std::endl;
    system(python_cmd_.str().c_str());
    ...
    for (int n = 0; n < num_slices; ++n){
        ...
        file_name << map_path_ << grid_dim << "_";
